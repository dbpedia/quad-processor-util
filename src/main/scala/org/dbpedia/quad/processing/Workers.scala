...
object Workers {
  ...
  /**
    ...
    */
  private[processing] val sentinel = new Object()
  ...
  def workInParallel[T, R](workers: Traversable[Workers[T, R]], args: Seq[T]): Unit = {
    try {
      for (worker <- workers) {
        worker.start()
        for (arg <- args)
...
class Workers[T, R](availThreads: Int, queueLength: Int, factory: Worker[T, R]) extends Closeable {
  ...
  private val queue = new ArrayBlockingQueue[T](queueLength)
  ...
  final def stop(): Unit = {
    ...
    for (thread <- threads) queue.put(sentinel.asInstanceOf[T])
