...
import org.dbpedia.quad.file.RichFile._
import org.dbpedia.quad.utils.UriUtils
...
class RichFile(file: File) extends FileLike[File] {
  ...
  def names(pattern: Pattern): List[String] = {
    val list = file.list(RichFile.filenameFilter(pattern))
  ...
  def list(pattern: Pattern): List[File] = {
    val list = file.listFiles(RichFile.filenameFilter(pattern))
    if (list == null) throw new IOException("failed to list files in ["+file+"]")
    list.toList
  }
