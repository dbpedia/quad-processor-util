...
class RdfNamespace(val prefix: String, val namespace: String, val validate: Boolean) {
  ...
  protected def append(sb: StringBuilder, suffix: String): Unit = {
    StringUtils.escape(sb, suffix, RdfNamespace.iriEscapes)
  }
...
object RdfNamespace {
  ...
  private val iriEscapes = {
    ...
    StringUtils.replacements('%', chars)
  }

  val prefixMap = new HashMap[String, RdfNamespace]
  val iriMap = new HashMap[String, RdfNamespace]
  private var newPrefixCount = 0

  def findPrefix(iri: String): RdfNamespace ={
    iriMap.get(iri) match{
      case Some(ns) => return ns
      case None => {
        for(ns <- iriMap.keySet)
          if(iri.contains(ns))
            return iriMap(ns)
        newPrefixCount = newPrefixCount +1
        ns("uns" + String.valueOf(newPrefixCount), iri, false)
      }
    }
  }
  
  private def ns(prefix: String, namespace: String, validate: Boolean = true): RdfNamespace = {
    iriMap.get(namespace) match {
      case None =>{
  ...
  for(n <- Config.UniversalConfig.prefixMap)
    ns(n._1, n._2)
